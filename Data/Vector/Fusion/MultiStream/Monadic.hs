...
mfoldlu p q z (MultiStream stepm step1 s) =
    mfoldluM_loopm SPEC (multireplicate z) s
  where
    mfoldluM_loopm !sPEC mz s
      = do
          r <- stepm s
          case r of
            Yield x s' -> do { let { mz' = q mz x }; mfoldluM_loopm SPEC mz' s' }
            Skip    s' -> mfoldluM_loopm SPEC mz s'
            Done       -> do { z' <- mfoldluM_loop1 SPEC z s 
                             ; return $ multifold p z' mz
                             }

    mfoldluM_loop1 !sPEC z s
      = do
          r <- step1 s
          case r of
            Yield x s' -> do { let { z' = p z x }; mfoldluM_loop1 SPEC z' s' }
            Skip    s' -> mfoldluM_loop1 SPEC z s'
            Done       -> return z
...
mfoldlu'  ::  (Monad m, MultiType a)
          ...
          ->  m a
{-# INLINE_FUSED mfoldlu' #-}
mfoldlu' p q z (MultiStream stepm step1 s) = mfoldluM_loopm SPEC (multireplicate z) s
  where
    mfoldluM_loopm !sPEC mz s
      = mz `seq` do
          r <- stepm s
